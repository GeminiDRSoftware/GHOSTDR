...
@pytest.mark.fullreduction
@pytest.mark.parametrize('arm,res', TestMasterFlat.ARM_RES_COMBOS)
def test_arc_missing_pixelmodel(arm, res, get_or_create_tmpdir):
    """
    Perform overscan subtraction on raw bias frame
    """
    # import pdb; pdb.set_trace()
    ...
    reduce.logfile = os.path.join(
        ...
        'reduce_masterarc_pixmoderror_{}_{}.log'.format(
            res, arm))
    reduce.logmode = 'quiet'
    reduce.suffix = '_{}_{}_testArcMissingPixelModel'.format(res, arm)
    logutils.config(file_name=reduce.logfile, mode=reduce.logmode)
    # import pdb; pdb.set_trace()
    ...
    del flat_ad[0].PIXELMODEL
    flatname = 'flat_nopixmod_{}_{}.fits'.format(arm, res)
    flat_ad.write(filename=flatname)
