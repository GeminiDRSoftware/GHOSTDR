...
def get_or_create_tmpdir(tmpdir_factory):
    ...
    # This old_test suite requires a minimum amount of available disk space.
    ...
    if get_free_space_mb(os.path.join(
        ...
        raise RuntimeError('You have insufficient free disk space to run '
                           'the full reduction old_test suite.')
    ...
    rawfiles = glob.glob(os.path.join(
        ...
        rawfilename))
    if len(rawfiles) == 0:
        raise RuntimeError("Test data for 'fullreduction' old_test suite is "
