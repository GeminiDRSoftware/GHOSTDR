...
class Extractor():
    ...
    takes a single string representing the configuration.
    ...
    every y (dispersion direction) coordinate and order, and a "w_map", which
    is the wavelength corresponding to every y (dispersion direction) coordinate
    and order. """
    ...
    def __init__(self, arm, mode):
        ...
        #Fill in the slit dimensions in "simulator pixel"s.
        # based on if we are in the
        ...
        pix_offset_ix = np.append(np.append([0],
                                            np.arange(1, self.nl).repeat(2)),
                                  self.nl)
        ...
        for i in range(self.nl):
            self.square_offsets[:,i] = (pix_offset_ix - self.nl/2.0) * \
                                       self.lenslet_width / self.matrices[
                i,self.x_map.shape[1]//2, 0, 0
            ]
        ...
        for i in range(nm):
            self.sim_offsets[:, i] = im_slit_pix_in_microns / self.matrices[
                i,self.x_map.shape[1]//2, 0, 0
            ]
        # To aid in 2D extraction, let's explicitly compute the
        # y offsets corresponding to these x offsets...
    ...
    def define_profile(self, fluxes):
        ...
        if fluxes.shape[0] != self.nl:
            raise ValueError("Error: %s resolution mode must"
                             " have %d lenslets" % (self.mode, self.nl, ))
        else:
    ...
    def one_d_extract(self, data=[], file='', badpix=[],
                      lenslet_profile='sim', rnoise=3.0):
        ...
        # Assuming that the data are in photo-electrons,
        # construct a simple model for the
        ...
        for i in range(nm):
            ...
            # Based on the profile we're using, create the local offsets
            # and profile vectors
            ...
            for j in range(ny):
                ...
                x_ix = int(self.x_map[i, j]) - nx_cutout//2 + np.arange(
                    nx_cutout,dtype=int
                ) + nx//2
                for k in range(no):
                    phi[:, k] = np.interp(x_ix - self.x_map[i, j] -
                                          nx//2, offsets, profile[:, k])
                    phi[:, k] /= np.sum(phi[:, k])
                ...
                # simple explicit way.
                col_inv_var_mat = np.reshape(col_inv_var.repeat(no),
                                             (nx_cutout, no))
                ...
                extracted_var[i, j, :] = np.dot(1.0/np.maximum(col_inv_var,
                                                               1e-12),
                                                pixel_weights**2)
        return extracted_flux, extracted_var
        
    def two_d_extract(self, file='', data=[], badpix=[], lenslet_profile='sim',
                      rnoise=3.0, deconvolve=True):
        ...
        for i in range(nm):
            ...
            if lenslet_profile == 'sim':
                ...
                raise ValueError("Only sim lenslet profile available "
                                   "for 2D extraction so far...")
            nx_cutout = 2 * int((np.max(offsets) -
                                 np.min(offsets))/2 ) + 2
            ny_cutout = 2*int(nx_cutout * np.nanmax(np.abs(self.slit_tilt)) /
                              2) + 3
            ...
            for j in range(ny):
                ...
                for k in range(no):
                    x_prof = np.interp(x_ix - self.x_map[i ,j] - nx//2,
                                       offsets, profile[:, k])
                    y_pix = (x_ix - self.x_map[i, j] -
                             nx//2) * self.slit_tilt[i, j] + ny_cutout//2
...
#                        pdb.set_trace()
                ...
                extracted_var[i, j, :] = np.dot(1.0/np.maximum(col_inv_var,
                                                ...
                                                pixel_weights**2)
                if j > 0:
                    extracted_covar[i, j-1, :] = np.dot(
                        1.0/np.maximum(col_inv_var, 1e-12),
...
#                    pdb.set_trace()
        if deconvolve:
            # Create the diagonals of the matrix Q gradually,
            # using the Taylor approximation for
            # the matrix inverse.
            ...
            a_diag_p1 = extracted_covar/extracted_sig[:, :-1, :]/\
                        extracted_sig[:, 1:, :]
#            a_diag_m1 = extracted_covar/extracted_var[:,1:,:]
            ...
            Q_diag_p2 = 3/8.0*a_diag_p1[:, :-1, :] * a_diag_p1[:, 1:, :]/ \
                        extracted_sqrtsig[:, 2:, :]/extracted_sqrtsig[:, :-2, :]
