...
class Arm():
    ...
    def simulate_image(self,x,w,b,matrices,im_slit,spectrum=None,nx=0, xshift=0.0, yshift=0.0, rv=0.0):
            ...
            if (spectrum is None) or len(spectrum)==0:
                d =pf.getdata(os.path.join(os.path.dirname(os.path.abspath(__file__)),'data/ardata.fits.gz'))
            ...
            for i in range(nm):
                for j in range(ny):
                    ...
                    ww = np.where( (slit_x>=0) * (slit_y>=0) * \
                                   (slit_x<im_slit.shape[1]) * (slit_y<im_slit.shape[0]) )
                    cutout_shifted = (cutout_shifted[0][ww], cutout_shifted[1][ww])
                    slit_x = slit_x[ww]
                    slit_y = slit_y[ww]
                    image[cutout_shifted[1],cutout_shifted[0]] += b[i,j]*flux*im_slit[slit_y,slit_x]
    ...
    def sky_background(self, mode):
        ...
        bgdata = np.loadtxt(os.path.join(os.path.dirname(os.path.abspath(__file__)),'data/skybg_50_10.dat'), skiprows=14)
        # Convert nm to um
        bgdata[:,0] /= 1000
    ...
    def simulate_frame(self, duration=0.0, output_prefix='test_', spectrum=None, xshift=0.0, yshift=0.0, rv=0.0, 
        rv_thar=0.0, flux=1e2, rnoise=3.0, gain=[1.0], bias_level = 10, overscan = 32, namps=[1,1],
        use_thar=True, mode='high', add_cosmic=True, add_sky=True, return_image=False, thar_flatlamp=False):
        ...
        spectrum: array of shape (2,n) where n is the number of spectral samples
            input spectrum. spectrum[0] is the wavelength array, spectrum[1] is the flux array.
        ...
        flux: float (optional) Flux multiplier for the reference spectrum to give photons/pix/s.
        ...
        bias_level: float (optional)
            Bias level in electrons, per amplifier (or a single scalar for all amps).
        ...
        if (mode == 'high'):
            ...
            image = self.simulate_image(x,w,b,matrices,im_slit, spectrum=spectrum, nx=self.szx, xshift=xshift,rv=rv)
            ...
            if (use_thar):
                ...
                image += self.simulate_image(x,w,b,matrices,im_slit2, spectrum=thar_spect, nx=self.szx, xshift=xshift,rv=rv_thar)
        else:
        ...
        if add_sky:
          ...
          if (mode == 'high'):
            ...
            im_slit2 = self.make_lenslets(fluxes=slit_fluxes, mode=mode, llet_offset=0)
          elif (mode == 'std'):
            slit_fluxes = np.ones(10)
            # FIXME llet_offset for standard mode?
            im_slit2 = self.make_lenslets(fluxes=slit_fluxes, mode=mode, llet_offset=0)
          sky_image = self.simulate_image(x, w, b, matrices, im_slit2, spectrum=sky_spect, nx=self.szx, xshift=xshift, rv=0.0)
        ...
        if add_cosmic:
          ...
          # A shield that only allows rays that originate within 10 degrees of the zenith
          # Pixel size of 15 x 15 x 16 um
          image += cosmic.cosmic(image.shape, duration, 10, 2.0, False, [15,15,16])

        # Add dark current (3 e/pix/hour)
        image += np.random.poisson(np.ones_like(image) * 3.0 * duration / 3600.0)

        # FIXME Multiply by flatfield map (i.e. simulate pixel-pixel non-uniformity)
        ...
        images = [i+r*np.random.normal(size=i.shape) for i,r in zip(images, rnoise)]
        ...
        # FIXME should convert to unsigned short here, and deal with saturation.

        # Now create our fits image!
        # By adding the DETSIZE and DETSEC keywords we can open the raw image in ds9 as an IRAF mosaic
        hdr = pf.Header()
        ...
        for i,im in enumerate(images):
          hdr = pf.Header()
          hdr['DETSIZE'] = "[1:%d,1:%d]" % (image.shape[1], image.shape[0])
          hdr['DETSEC'] = "[%d:%d,%d:%d]" % (cxl[i]+1, cxh[i], cyl[i]+1, cyh[i])
          hdr['DATASEC'] = "[%d:%d,%d:%d]" % (1, im.shape[1]-overscan, 1, im.shape[0])
          hdr['TRIMSEC'] = "[%d:%d,%d:%d]" % (1, im.shape[1]-overscan, 1, im.shape[0])
          if overscan > 0:
              hdr['BIASSEC'] = "[%d:%d,%d:%d]" % (im.shape[1]-overscan+1, im.shape[1], 1, im.shape[0])
