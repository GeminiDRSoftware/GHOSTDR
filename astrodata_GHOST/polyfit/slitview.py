...
class SlitView(object):
    def __init__(self, slit_image, microns_pix=4.54*180/50*2, \
        ...
        slit_image: numpy array
            A single slit viewer image, which has been processed, cosmic-ray rejection
            etc.
            
        microns_pix: float (optional)
            Scale in microns in the slit plane for each pixel in the slit viewing camera.
            The default value assumes 2x2 binning of the slit viewing camera.
        ...
        the y-axis slit coordinate (or, if needed, y-axis pixels) corresponding to 
        the boundaries of object 1 and 2 fluxes.
        """
    ...
    def slit_profile(self, arm='red', return_centroid=False):
        ...
        cutout = self.slit_image[central_pix[0]-y_halfwidth:central_pix[0]+y_halfwidth+1,\
            central_pix[1]-self.extract_half_width:central_pix[1]+self.extract_half_width+1]
        profile = np.sum(cutout, 1)
        if return_centroid:
            xcoord = np.arange(-self.extract_half_width, self.extract_half_width+1)
            xcoord = np.tile(xcoord, 2*y_halfwidth+1).\
                reshape( (2*y_halfwidth+1,2*self.extract_half_width+1) )
