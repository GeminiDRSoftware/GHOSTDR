...
#                                                               AstroDataType.py
# ------------------------------------------------------------------------------
# $Id: AstroDataType.py 5275 2015-06-11 14:47:10Z kanderson $
# ------------------------------------------------------------------------------
__version__      = '$Revision: 5275 $'[11:-2]
__version_date__ = '$Date: 2015-06-12 00:47:10 +1000 (Fri, 12 Jun 2015) $'[7:-2]
# ------------------------------------------------------------------------------
"""
...
from .Requirements import AND, OR, NOT
...
def re_header_keys(rekey, header):
    ...
    """
    retset = []
    if astrodata.new_pyfits_version:
...
class PHUReq(Requirement):
    ...
    def satisfied_by(self, hdulist):
        ...
        for reqkey in self.phuReqs.keys():
            ...
            # get list of keys
            if (mods_re):
                cleanreqkeylist = re_header_keys(cleanreqkeyPRE, 
                                                 hdulist[0].header)
            ...
            no_match_msg  = "FILE Header DID NOT match %s with value of %s"
            for cleanreqkey in cleanreqkeylist:
                if cleanreqkey in phuCardsKeys:
                    try:
                        ...
                        if (match):
                            if (verbose) : 
                                print "FILE header matched %s with value: %s" % \
                                    (reqkey, phuCards[reqkey])
