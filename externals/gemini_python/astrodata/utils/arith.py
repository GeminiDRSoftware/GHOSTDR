...
#                                                                       arith.py
# ------------------------------------------------------------------------------
# $Id: arith.py 5274 2015-06-11 14:39:37Z kanderson $
# ------------------------------------------------------------------------------
...
import astrodata
from .Errors import ArithError
from .gemconstants import SCI, VAR, DQ
from ..interface.Descriptors import DescriptorValue
...
def determine_inputs(input_a=None, input_b=None):
    ...
    if (isinstance(input_b, DescriptorValue) or isinstance(input_b, dict)):
    ...
    # Check to see if input_b is an AstroData object
        ...
        for ext in input_a[SCI]:
            ...
            if (input_a[(SCI, extver)].data.shape !=
                ...
                raise ArithError(msg)
            
            # Return the AstroData object
            return_value = input_b
    
    else:
        raise ArithError("Unknown type for input_b")
    return return_value
...
def propagate_variance(input_a=None, input_b=None, operation=None):
    ...
    for var in input_a[VAR]:
        ...
        if operation == "add":
            ...
            raise ArithError("The operation parameter must have a value of "
