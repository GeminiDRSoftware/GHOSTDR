...
from .utils.netutil import urlfetch
from .utils.gemutil import rename_hdu
from .utils.docstrings import SUBDATA_INFO_STRING
from .utils.mkcalciface import get_calculator_interface

from .interface.AstroDataType import *  # Not recommended -> ClassificationLibrary
...
class AstroData(CalculatorInterface):
    ...
    :param exts: (advanced) a list of extension indexes in the parent
                 'HDUList' that this instance should refer to, given  integer 
                 or (EXTNAME, EXTVER) tuples specifying each extension in the 
                 pyfits index space where the PHU is at index 0, the first 
                 data extension is at index 1, and so on. I.e. This is primarily 
                 intended for internal use when creating "sub-data", which are 
                 AstroData instances that represent a slice, or subset, of some 
                 other AstroData instance.
                 ...
                 HDUs in this list would still be shared (in memory) with the 
                 'ad' object, and appear in its ``HDUList`` as well.
    :type exts: list
        
    :param extInsts: (advanced) A list of extensions this instance should 
                     contain, specified as actual pyfits.HDU instances. 
                     NOTE: if the 'exts' argument is also set, ``extInsts`` 
                     is ignored.
    ...
    :param mode: IO access mode, same as pyfits mode ("readonly", "update",
                 ...
                 constructor checks that the named file does not exist on disk,
                 and if it does not it creates an empty ``AstroData`` of that 
                 name but does not write it to disk. Such an ``AstroData`` 
                 instance is ready to have HDUs appended, and to be written to 
                 disk at the user's command with ``ad.write()``.
    :type mode: string
    ...
    def __getitem__(self, ext):
        ...
        if type(ext) is str:
            ...
            return AstroData(self, exts=[ext])

        elif isinstance(ext,  types.FunctionType):
            hdus = ext(self)
            return AstroData(self, extInsts=hdus)
    ...
    def append(self, moredata=None, data=None, header=None, extname=None, 
        ...
        if moredata:
            ...
            try:
                ...
                err_msg = "parameter header must be <type pyfits.Header>."
                raise Errors.AstroDataError(err_msg)
            try:
                assert isinstance(data, numpy.ndarray)
            except AssertionError:
                err_msg = "parameter data must be <type numpy.ndarray>"
                raise Errors.AstroDataError(err_msg)

            self._onehdu_work(append=True, header=header, data=data,
                             extname=extname, extver=extver, autonum=auto_number)
        self.phu_set_key_value('NEXTEND', self.count_exts())
        return
    ...
    def write(self, filename=None, clobber=False, rename=None, prefix=None, 
        ...
        """
        # Set NEXTEND correctly. Callers may have added or removed extensions
        # outside the knowledge of this API.
        self.phu_set_key_value('NEXTEND', self.count_exts())
    ...
    def _check_for_simple_fits_file(self, source):
        ...
        if not hasattr(first_extension, 'data'):
            first_extension.data = None
    ...
    def _infostr(self, as_html=False, oid=False, table=False, help=False):
        ...
        if not as_html:
            ...
            for hdu in self.hdulist[1:]:
                ...
                if extType == "ImageHDU":
                    if self.hdulist[hdu_indx].data is None:
                        dimention_ = None
                        form_ = None
                    else:
